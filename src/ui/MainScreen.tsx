    ...
    <>
            ...
            <button
              ...
              onClick={() => {
                if (focus.kind === "hostileFleet") {
                  ...
                  // Focus the idle fleet's system and enter move mode
                  // on it. The move-bar appears in the system pane
                  // (no more galaxy overlay), so we stay on / swap to
                  // the system view rather than the galaxy.
                  setSelectedSystemId(focus.systemId);
      ...
      <div className={`main-column mobile-view-${mobileView}`}>
        <div className="system-panel">
          ...
          {/* Move-mode controls — lives IN the system pane so it no
              longer overlays the galaxy map. Shown as a band between
              the fleet strip and the body list while a fleet is
              selected for movement. On mobile the user taps "Galaxy"
              in the header to switch to the map and pick a target;
              handleSystemSelect swaps them back on commit. */}
          {moveFleet && !moveFleetStale && (() => {
            const dest = moveFleet.destinationSystemId
              ? state.galaxy.systems[moveFleet.destinationSystemId] ?? null
              : null;
